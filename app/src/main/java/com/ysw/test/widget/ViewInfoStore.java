...
class ViewInfoStore {
     ...
     */
    @Nullable
    RecyclerView.ItemAnimator.ItemHolderInfo popFromPreLayout(ViewHolder vh) {
     ...
     */
    @Nullable
    ItemAnimator.ItemHolderInfo popFromPostLayout(ViewHolder vh) {
        return popFromLayoutStep(vh, InfoRecord.FLAG_POST);
    }

    private ItemAnimator.ItemHolderInfo popFromLayoutStep(ViewHolder vh, int flag) {
        ...
        if (record != null && (record.flags & flag) != 0) {
            record.flags &= ~flag;
            final ItemAnimator.ItemHolderInfo info;
     ...
     * @param info The information to save
     */
    void addToAppearedInPreLayoutHolders(com.ysw.test.widget.RecyclerView.ViewHolder holder, ItemAnimator.ItemHolderInfo info) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            record = InfoRecord.obtain();
            mLayoutHolderMap.put(holder, record);
     ...
     * @param info The information to save
     */
    void addToPostLayout(ViewHolder holder, ItemAnimator.ItemHolderInfo info) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            record = InfoRecord.obtain();
            mLayoutHolderMap.put(holder, record);
    ...
    interface ProcessCallback {
        void processDisappeared(ViewHolder viewHolder, @NonNull ItemAnimator.ItemHolderInfo preInfo,
                                @Nullable ItemAnimator.ItemHolderInfo postInfo);
        void processAppeared(ViewHolder viewHolder, @Nullable ItemAnimator.ItemHolderInfo preInfo,
